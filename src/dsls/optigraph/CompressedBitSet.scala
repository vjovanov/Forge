...
trait CompressedBitSetOps {
  ...
  def importCompressedBitSetOps() {
    ...
    */
    ...
    static (CompressedBitSet) ("apply", Nil, MArray(MInt) :: CompressedBitSet) implements allocates(CompressedBitSet,${array_length($0)},${cbs_alloc_level_1($0)},${cbs_alloc_from_int_array($0)})
    ...
    CompressedBitSetOps{
      ...
      infix ("andCardinality") (("bs2",CompressedBitSet) :: MLong) implements single ${
        val bs1_level1 = NodeData(getLevel1($self))
        val bs2_level1 = NodeData(getLevel1($self))

        val bs1_level2 = NodeData(getLevel2(bs2))
        val bs2_level2 = NodeData(getLevel2(bs2))
        var count = 0l

        var bs1_l1 = 0
        var bs2_l1 = 0
        while(bs1_l1 < bs1_level1.length && bs2_l1 < bs2_level1.length){
          if(bs1_level1(bs1_l1) < bs2_level1(bs2_l1)){
            ...
            val bs1_level2_keys = NodeData(fhashmap_keys(bs1_level2(bs1_l1)))
            val bs2_level2_keys = NodeData(fhashmap_keys(bs2_level2(bs2_l1)))

            var bs1_l2 = 0
            var bs2_l2 = 0 
            while(bs1_l2 < bs1_level2_keys.length && bs2_l2 < bs2_level2_keys.length){
              if(bs1_level2_keys(bs1_l2) > bs2_level2_keys(bs2_l2)){
                ...
                if(nc1.colType == 1){
                  if(nc2.colType ==1){
                    NodeDataView(nc_getNodeData(nc1).getRawArray,0,nc_getNodeData(nc1).length).intersect(NodeDataView(nc_getNodeData(nc2).getRawArray,0,nc_getNodeData(nc2).length))
                  }
                  else{
                    intersect(258,NodeDataView(nc_getNodeData(nc1).getRawArray,0,nc_getNodeData(nc1).length),nc_getgraphbitset(nc2))
                  }
                }
                else{
                  if(nc2.colType ==1){
                    intersect(258,nc_getgraphbitset(nc1),NodeDataView(nc_getNodeData(nc2).getRawArray,0,nc_getNodeData(nc2).length))
                  }
                  else{
