...
trait NodeDataOps {
  ...
  def importNodeDataOps() {
    ...
    static(NodeData)("apply", T, MLong :: NodeData(T)) implements allocates(NodeData,${array_buffer_strict_empty[T]($0)})
    ...
    static(NodeData)("fromFunction", T, (MLong,(MLong ==> T)) :: NodeData(T)) implements allocates(NodeData,${array_buffer_new_imm(array_fromfunction($0,$1))})

    val NodeDataOps = withTpe(NodeData)
    NodeDataOps{  
      //////////////basic accessors//////////////////////////////
      infix("apply")(MLong :: T) implements composite ${array_buffer_apply(nd_raw_data($self),$1)}
      infix("update")( (("id",MLong),("n",T)) :: MUnit, effect=write(0)) implements composite ${array_buffer_update(nd_raw_data($self),$id,$n)}
      infix ("length")(Nil :: MLong) implements single ${array_buffer_length(nd_raw_data($self))}
      ...
      infix("sort")(Nil :: NodeData(T),TNumeric(T)) implements composite ${NodeData(array_sort($self.getRawArray))}      
      infix ("sortBy") ((MLong ==> R) :: NodeData(T), TOrdering(R), addTpePars=R) implements composite ${
          NodeData[Long](array_sortIndices($self.length,$1)).map[T](i => $self(i))
      }
      infix ("sortIndicesBy") ((MLong ==> R) :: NodeData(MLong), TOrdering(R), addTpePars=R) implements single ${
          NodeData[Long](array_sortIndices($self.length,$1))
      }

      /////////////////////////debug operations (print serial & parallel)///////////////////////
      infix ("pprint") (Nil :: MUnit, effect = simple) implements foreach(T, 0, ${a => println("NodeData: " + a)})
      infix ("forindicies") ((MLong ==> MUnit) :: MUnit, effect = simple) implements composite ${
      ...
      compiler("nd_raw_alloc")(MLong :: NodeData(R), addTpePars = R, effect=mutable) implements composite ${NodeData[R]($1)}
      compiler ("nd_apply") (MLong :: T) implements composite ${array_buffer_apply(nd_raw_data($self), $1)}
      compiler("nd_update")( (("id",MLong),("n",T)) :: MUnit, effect=write(0)) implements composite ${array_buffer_update(nd_raw_data($self),$id,$n)}
      compiler ("nd_set_length")(MLong :: MUnit, effect = write(0)) implements single ${array_buffer_set_length(nd_raw_data($self),$1)}
      compiler ("nd_set_raw_data") (MArrayBuffer(T) :: MUnit, effect = write(0)) implements setter(0, "_data", quotedArg(1))
      compiler ("nd_appendable") ((MLong,T) :: MBoolean) implements single("true") 
      compiler ("nd_append") ((MLong,T) :: MUnit, effect = write(0)) implements composite ${array_buffer_append(nd_raw_data($self),$2)}
      compiler("nd_copy") ((MLong,NodeData(T),MLong,MLong) :: MUnit, effect = write(2)) implements single ${array_buffer_copy(nd_raw_data($self),$1,nd_raw_data($2),$3,$4)}
